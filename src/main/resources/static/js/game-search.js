document.addEventListener('DOMContentLoaded', () => {
    ...
    const metacriticDisplay = document.getElementById('metacritic-display');
    ...
    function renderSuggestions(results) {
        ...
        results.forEach(game => {
            ...
            const rating = game.externalRating != null ? `MC ${game.externalRating}/100` : '';
    ...
    function selectGame(game) {
        ...
        if (metacriticDisplay && game.externalRating != null) {
            const source = game.ratingSource === 'METACRITIC' ? 'Metacritic'
                : game.ratingSource === 'CATALOG' ? 'Metacritic (catálogo)'
                : game.ratingSource === 'STEAM' ? 'Steam' : 'Referência';
            metacriticDisplay.textContent = `${game.externalRating}/100 · ${source}`;
        }
        ...
        hideSuggestions();
        if (searchHint) {
            searchHint.textContent = 'Jogo selecionado! Capa e nota Metacritic serão aplicadas ao salvar.';
        }
    }
