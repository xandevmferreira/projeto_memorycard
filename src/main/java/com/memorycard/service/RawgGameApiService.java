...
public class RawgGameApiService implements ExternalGameApiService {
    ...
    private ExternalGameInfo mapToExternalGameInfo(JsonNode node) {
        ...
        BigDecimal metacritic = null;
        String ratingSource = null;
        if (node.has("metacritic") && !node.get("metacritic").isNull() && node.get("metacritic").asInt(0) > 0) {
            metacritic = BigDecimal.valueOf(node.get("metacritic").asInt());
            ratingSource = "METACRITIC";
        } else if (node.has("rating") && !node.get("rating").isNull()) {
            metacritic = BigDecimal.valueOf(node.get("rating").asDouble() * 20)
        ...
        return new ExternalGameInfo(title, platform, metacritic, ratingSource, coverUrl, releaseDate, platforms);
    }
